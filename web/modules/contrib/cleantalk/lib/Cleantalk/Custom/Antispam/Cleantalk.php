...
class Cleantalk
{
    ...
    private function filterRequest(CleantalkRequest $request)
    {
        ...
        foreach ($request as $param => $value) {
            if (in_array(
                $param, array('message', 'example', 'agent',
                'sender_info', 'sender_nickname', 'post_info', 'phone')
            ) && !empty($value)
            ) {
    ...
    private function compressData($data = '')
    {
        ...
        if (is_string($data) && strlen($data) > $this->dataMaxSise && function_exists('gzencode') && function_exists('base64_encode')) {
    ...
    {
        if (is_string($str) && !preg_match('//u', $str) && function_exists('mb_detect_encoding') && function_exists('mb_convert_encoding')) {
